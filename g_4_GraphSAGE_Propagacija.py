...
# Varijanta: GraphSAGE embedding (mean aggregate, strukturni početni featurei)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
GraphSAGE = sample & aggregate (ovde: 1 sloj mean agregacije, bez učenja težina)
"""
...
Graf iz M (networkx). 
Početni feature po čvoru: 
stepen (normalizovan) + (k-1) 
sopstvenih vektora Laplaciana → matrica (39, k). 
Jedan sloj mean agregacije: 
h_v = x_v + prosek(x_u za u u susedima). 
Rezultat se normalizuje u [0, π]. 

Samo numpy i networkx.
"""
...
def compute_graphsage_embeddings(M, k=EMBED_DIM):
    G = nx.Graph()
    for i in range(1, 40):
        G.add_node(i)
    for i in range(1, 40):
    ...
    degrees = M.sum(axis=1)[1:40].astype(float)
    deg_min, deg_max = degrees.min(), degrees.max()
    if deg_max - deg_min > 0:
        deg_norm = (degrees - deg_min) / (deg_max - deg_min)
    else:
        deg_norm = np.zeros(39)

    L = np.diag(M.sum(axis=1)) - M
    L_sub = L[1:40, 1:40]
    _, vecs = np.linalg.eigh(L_sub)
    if k - 1 <= vecs.shape[1] - 2:
        eig_feat = vecs[:, 1:1 + (k - 1)]
    else:
        eig_feat = vecs[:, 1:2].repeat(k - 1, axis=1)
    x0 = np.hstack([deg_norm.reshape(-1, 1), eig_feat]).astype(float)
    ...
    for i in range(39):
        ...
        if neigs:
            idx_neig = [u - 1 for u in neigs]
            mean_neig = x0[idx_neig].mean(axis=0)
            h[i] = x0[i] + mean_neig
        else:
            h[i] = x0[i]
...
def main():
    ...
    emb = compute_graphsage_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (GraphSAGE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    Predikcija (GraphSAGE + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 34, 35)
    """
